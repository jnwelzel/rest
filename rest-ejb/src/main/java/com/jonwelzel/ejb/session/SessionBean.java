...
/**
 * Stateless session bean exposing user session-related operations. The
 * information is all stored in Redis.
 * 
 * @author jwelzel
 * 
 */
...
public class SessionBean {
	...
	/**
	 * Find the {@link User}'s {@link AuthToken} ID value using the session key
	 * associated to it.
	 * 
	 * @param sessionKey The session token stored in Redis that was generated
	 *            when the user logged in.
	 * @return The user's auth token id value associated to this session or null
	 *         if the session expired (doesn't exist).
	 */
	...
	/**
	 * Checks the validity of a given session. If the session has a TTL updates
	 * it with the value of {@link #SESSION_TIMEOUT}.
	 * 
	 * @param key Session unique identifier.
	 * @return -1 if the session with given key doesn't exist, 0 if session
	 *         timed out or 1 if all is good.
	 */
	...
	/**
	 * Create a new session for the user. The key is a secure random hash and
	 * the value is the secure random hash that identifies the user, which is
	 * created automatically when his account is created.
	 * 
	 * @param key Secure random hash value that will be used to identify this
	 *            session.
	 * @param userId Secure random hash value that will be used to identify the
	 *            user.
	 * @param remember If true the session will have no expire date, if false it
	 *            will have a {@link #SESSION_TIMEOUT}.
	 */
	 ...
	 * 
	 * @param key Session unique identifier.
	 * @return -1 if error occurred, 0 if no session was expired or 1 if all
	 *         went well.
	 */
